...
class MercadoLivreProvider(Provider):
    ...
    def search(self, query: NormalizedQuery) -> ProviderResult:
        ...
        if query.condition != Condition.ALL:
            params["condition"] = query.condition.value

        headers = {
            "Accept": "application/json",
            "User-Agent": "PersonalDealIntelligence/1.0",
        }
        ...
        try:
            ...
            return ProviderResult(
                ...
                [],
                "Timeout ao consultar Mercado Livre.",
                retrieved_at,
            )
            ...
            return ProviderResult(
                ...
                [],
                f"Erro de rede: {exc.__class__.__name__}",
                retrieved_at,
            )
        ...
        if resp.status_code == 401:
            return ProviderResult(
                ...
                [],
                "Mercado Livre exige um Access Token válido para esta chamada.",
                retrieved_at,
            )
        if resp.status_code == 403:
            message = (
                "Mercado Livre retornou 403. Configure ML_ACCESS_TOKEN nos Secrets "
                "do Streamlit com um token oficial válido para o app Mercado Livre."
            )
            return ProviderResult(self.name, ProviderStatus.UNAVAILABLE, [], message, retrieved_at)
        ...
        if resp.status_code >= 500:
            return ProviderResult(
                ...
                ProviderStatus.ERROR,
                [],
                f"Erro do servidor Mercado Livre ({resp.status_code}).",
                retrieved_at,
            )
        if not resp.ok:
            return ProviderResult(
                ...
                ProviderStatus.ERROR,
                [],
                f"Resposta inesperada ({resp.status_code}).",
                retrieved_at,
            )
        ...
        for item in payload.get("results") or []:
            ...
            if offer is None or not is_compatible(offer.match_level):
                continue
            if query.max_price is not None and offer.effective_price is not None and offer.effective_price > query.max_price:
                continue
            offers.append(offer)

        offers.sort(
            key=lambda o: (
                0 if o.match_level == MatchLevel.EXACT_MATCH else 1,
                o.effective_price if o.effective_price is not None else 1e12,
                0 if o.free_shipping else 1,
            )
        )
    ...
    def _map_item(self, item: dict[str, Any], query: NormalizedQuery, retrieved_at: datetime) -> Optional[Offer]:
        ...
        if isinstance(thumbnail, str) and thumbnail.startswith("http"):
            ...
            image_url = thumbnail if isinstance(thumbnail, str) else None
        ...
        return Offer(
            ...
            provider=self.name,
            condition=condition,
            match_level=match_offer(title, query),
            effective_price=calculate_effective_price(price, shipping),
